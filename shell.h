...
void exec_program(char *line, char **env, int line_number);
/*-------------End ain f);unctions---------*/
/*-------------Helper functions---------*/
void handl_signal(int sig);
int just_spaces(char *command);
void handl_exit(char *line, int line_number, list_cmd *my_list);
void handl_env(char **env);
void handl_chdir(char *line, int line_number, list_cmd *sep_cmds);
void handl_pwd();
